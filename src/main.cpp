...
void generateScansFromDegrees(std::vector<Camera> &scans, PointCloud &pc)
{
    ...
    int num_scans = floor(360/degrees_input);
...
void writeReconstructedMeshToFile(std::vector<Camera> const &scans,Eigen::MatrixXd &points_all_scans,PointCloud &reconstruction_cloud)
{
